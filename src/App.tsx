...
function HomeReadingDock({ showGloss, onOpenFreeReading, onOpenPrayer }: { showGloss: boolean; onOpenFreeReading: () => void; onOpenPrayer: (entry: OfficeEntry) => void }) {
  return <section className="home-reading-dock" aria-label="Additional reading and prayers"><button className="free-reading-panel" type="button" onClick={onOpenFreeReading}><span className="free-reading-heading"><span>Ἀνάγνωσις</span>{showGloss && <small>Free reading</small>}</span><img className="free-reading-icon canonical-icon" src={freeReadingIcon} alt="" aria-hidden="true"/></button><MealPrayerDock showGloss={showGloss} onOpen={onOpenPrayer}/></section>
}
