...
type ReaderOptions = { showGloss: boolean; darkMode: boolean; showProgressive: boolean; showChallenge: boolean; showPsalm: boolean }
...
function loadOptions(): ReaderOptions {
  try {
    ...
    return stored ? { ...DEFAULT_OPTIONS, ...(JSON.parse(stored) as Partial<ReaderOptions>) } : DEFAULT_OPTIONS
...
function MealPrayerDock({ showGloss, onOpen }: { showGloss: boolean; onOpen: (entry: OfficeEntry) => void }) {
  ...
  return <section className="meal-prayers" aria-labelledby="meal-prayers-title"><div className="meal-prayers-heading" id="meal-prayers-title"><span>Εὐχαριστίαι τραπέζης</span>{showGloss && <small>Meal prayers</small>}</div><nav className="meal-prayer-dock" aria-label="Meal prayers">{mealPrayers.map((prayer,index)=><button className="meal-prayer-button" type="button" key={prayer.id} onClick={()=>onOpen(prayer)}><MealIcon kind={icons[index]}/><span className="meal-prayer-label"><span>{labels[index].greek}</span>{showGloss && <small>{labels[index].english}</small>}</span></button>)}</nav></section>
}
...
export default function App() {
  ...
  useEffect(()=>{ localStorage.setItem(OPTIONS_KEY,JSON.stringify(options)); document.documentElement.dataset.theme=options.darkMode?'dark':'light' },[options])
